  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        if(toget!=0)
        // cprintf("The Burst Time is %d and pid is %d\n",currbursttime,toget->pid);
        if(currbursttime!=-1){
        p=toget;
        pid = p->pid;
        kfree(p->kstack);
...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    struct proc* chosenProc=&ptable.proc[0];
    int currbursttime=-1;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
    ...
    if(currbursttime!=-1){
      ...
      // cprintf("PID %d   BURTSTIME %d\n",chosenProc->pid,chosenProc->burstTime);
      p=chosenProc;
      c->proc = p;
      switchuvm(p);
      p->state = RUNNING;
...
int getProcInfo(int pid,struct processInfo* ptr){
  ...
  for(int i=0;i<NPROC;i++){
    if(ptable.proc[i].state!=UNUSED && ptable.proc[i].pid==pid){
      ...
      cprintf("Process Name : %s , Burst Time : %d, ",ptable.proc[i].name,ptable.proc[i].burstTime);
      break;
    }
  }
  release(&ptable.lock);
