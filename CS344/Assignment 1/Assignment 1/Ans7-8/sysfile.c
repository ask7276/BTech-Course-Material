// Added the sys_wolfie from 460-494
...
int sys_wolfie(void)
{
  ...
  int sizeofart = 0;
  // loop to find the number of characters in the ascii art
  ...
  if (argptr(0, &buffer, buffer_size) < 0)
    return -1;
  // if sizeofart is more than buffer exit
  if (sizeofart > buffer_size)
    return -1;
  // copies the ascii art to the user supplied buffer
