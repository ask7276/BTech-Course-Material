...
class DatasetLoader(dataset):
    ...
    def load_hop_wl_batch(self):
        ...
        f = open('./result/Hop/hop_' + self.dataset_name + '_' + str(self.k) + '_' + str(self.alpha), 'rb')
        hop_dict = pickle.load(f)
        f.close()

        print('Load Subgraph Batches')
        f = open('./result/Batch/' + self.dataset_name + '_' + str(self.k) + '_' + str(self.alpha), 'rb')
    ...
    def load(self):
        ...
        eigen_adj = None
        if self.compute_s:
            temp = features.todense()
            temp2 = temp.T
            attr_adj = np.dot(temp, temp2)
            attr_adj = sp.coo_matrix(attr_adj)
            temp3 = self.adj_normalize(adj)
            attr_adj = self.adj_normalize(attr_adj)
            temp_adj = self.alpha * temp3 + (1 - self.alpha) * attr_adj
            eigen_adj = self.c * inv((sp.eye(adj.shape[0]) - (1 - self.c) * self.adj_normalize(temp_adj)).toarray())
